...
export const patientRegister=catchAsyncErrors(async(req,res,next)=>{
    const {
        firstName,lastName,email,phone,password,gender,dob,nic,role}=req.body;
...
export const login= catchAsyncErrors(async(req,res,next)=>{
    ...
    if(role!=user.role){
        return next(new ErrorHandler("Role didnot match",400))
    }
    ...
    export const addNewAdmin=catchAsyncErrors(async(req,res,next)=>{
        ...
        if(!firstName || !lastName || !email || !phone || !password || !gender || !dob ||!nic){
            return next(new ErrorHandler("Please fill full form",100))
        }
...
export const addNewDoctor=catchAsyncErrors(async(req,res,next)=>{
    ...
    if(!firstName ||
        ...
        if(!cloudinaryResponse || cloudinaryResponse.error){
            console.error("Cloudinary Error:".cloudinaryResponse.error || "Unkwon cloudinary error")
        }
